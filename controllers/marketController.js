...
exports.add = (req, res)=>{
    ...
    if(name == "" || cost < 0 || quantities < 0 || discount < 0 ){
        return res.status(500).send({status: 500, message:"Input invalid. Must have field name, cost >= 0, quantities >=0 "}) 
    }
    var data = {
        name : name,
        cost: cost,
...
exports.buy = (req, res)=>{
    ...
    var currentUser = req.currentUser
    var quantities = parseInt(body.quantities) || 0
        ...
        if(items.length == 1){
            userDB.getFromId(currentUser._id,(err,user)=>{
                if(err)
                    return res.status(400).send({status: 400, message:err.message}) 
                if(user){
                    var total = quantities * items[0].cost;
